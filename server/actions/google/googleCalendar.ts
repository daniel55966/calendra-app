...
async function getOAuthClient(clerkUserId: string) {
  try {
    ...
    // Fetch the OAuth token for the given Clerk user ID
...
export async function createCalendarEvent({
  ...
  try {
    ...
    console.error("Error creating calendar event:", err.message || err); // Log the error to the console
    throw new Error(`Failed to create calendar event: ${err.message || err}`); // Throw a new error with a detailed message
  }
}
